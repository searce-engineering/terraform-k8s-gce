...
echo "checkpoint repository added"

sudo apt-get remove sshguard -y
sudo apt-get remove --auto-remove sshguard -y

echo "checkpoint ssh-guard uninstalled"

sudo apt-get update

echo "checkpoint apt-get updated"
...
echo "checkpoint network plugin set to cni"
...
echo "checkpoint 20-gcenet.cong edited"
...
echo "checkpoint gce conf configured"
...
echo "checkpoint docker kubelet kubeadm kubectl installed"
...
echo "checkpoint docker storage-driver set to overlay2"
systemctl restart docker


echo "checkpoint docker service restarted"
...
echo "checkpoint node labels added"
